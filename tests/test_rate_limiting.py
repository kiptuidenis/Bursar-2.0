...
def test_settings_update_rate_limiting_triggers_429(test_db, monkeypatch):
    """Firing 10 rapid settings update requests succeeds; 11th attempt triggers 429."""
    monkeypatch.setattr(config, "IS_TEST_MODE", False)
    if hasattr(app.state, "limiter"):
        monkeypatch.setattr(app.state.limiter, "enabled", True)
    ...
    app.dependency_overrides[get_db] = _db_override(test_db)
    user_id = test_db.create_user("254799000666", "pinpassword")
    app.dependency_overrides[get_current_user_id] = lambda: user_id

    try:
        client = TestClient(app)
        settings_payload = {"phone_number": "254799000666"}
        ...
        # 11th attempt -> 429 Too Many Requests
        res_limit = client.post("/api/settings", json=settings_payload)
        assert res_limit.status_code == 429, f"11th attempt expected 429, got {res_limit.status_code}"
