...
async def test_scheduler_does_not_trigger_before_scheduled_time(db):
    ...
    triggered = await check_and_trigger_payout(db, current_time, user_id=user_id)
...
async def test_scheduler_triggers_after_scheduled_time(db):
    ...
    mock_res = {
        ...
        "OriginatorConversationID": "mock_conv_123"
    }
    
    with patch("app.services.scheduler.send_b2c_payout", new_callable=AsyncMock) as mock_send:
...
async def test_scheduler_date_bounds(db):
    ...
    mock_res = {
        ...
        "OriginatorConversationID": "mock_conv"
    }
    ...
    with patch("app.services.scheduler.send_b2c_payout", new_callable=AsyncMock) as mock_send:
        ...
        assert await check_and_trigger_payout(db, t1, user_id=user_id) is False
        
        # Case 2: During active range (should trigger)
        t2 = datetime.datetime(2026, 6, 21, 8, 5, 0)
        assert await check_and_trigger_payout(db, t2, user_id=user_id) is True
        
        # Reset balance and payouts for Case 3
        db.update_settings(user_id, balance=1000.0)
        # Clear payouts for user
        conn = db.connection
        cursor = conn.cursor()
        cursor.execute("DELETE FROM payouts WHERE user_id = ?", (user_id,))
        conn.commit()
        
        # Case 3: After end date (should not trigger)
        t3 = datetime.datetime(2026, 6, 26, 8, 5, 0)
        assert await check_and_trigger_payout(db, t3, user_id=user_id) is False
